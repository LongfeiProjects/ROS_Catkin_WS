...
from ur_msgs.srv import SetPayload, SetIO, GripperMove
...
############################################################
#last_joint_torques = None
#last_joint_torques_lock = threading.Lock()
#pub_joint_torques = rospy.Publisher('joint_torques', JointsTorque.msg.float64, queue_size=1)
############################################################
...
class URConnection(object):
    ...
    def __on_packet(self, buf):
        ...
        msg = IOStates()
        #gets safety state
        msg.master_safety_state = state.masterboard_data.master_safety_state
        msg.master_onoff_state = state.masterboard_data.master_onoff_state
...
###############################################################
       # jointTrq_msg = JointsTorque()
       # jointTrq_msg.header.stamp = now
###############################################################
    ...
    def handle(self):
        ...
        try:
            ...
            while True:
                ...
                elif mtype == MSG_GRIPPER_MOVE_FINISHED:
                    self._gripper_motion_completed = True
                elif mtype == MSG_GRIPPER_OBJ_DETECTED:
                    self._gripper_object_detected = True
                else:
...
#############################################################################
    def rq_get_joint_torques(self):
        self.__send_message([MSG_GET_TORQUES])
#############################################################################

    def send_gripper_move(self,pos, force, speed, compensation):
        self._gripper_motion_completed = False
        self._gripper_object_detected = False

        if compensation:
            self.__send_message([MSG_GRIPPER_MOVE_COMPENSATION, pos, force, speed])
        else:
            self.__send_message([MSG_GRIPPER_MOVE, pos, force, speed])

        while not self._gripper_motion_completed:
            time.sleep(IO_SLEEP_TIME)

        return [True, self._gripper_object_detected]

    def send_gripper_activate(self):
        self.__send_message([MSG_GRIPPER_ACT])
...
class GripperServiceProvider(object):
    def __init__(self, robot):
        #print "gripper service up"
        self.robot = robot
        rospy.Service('gripper/move', GripperMove, self.moveGripper)
        rospy.Service('gripper/activate', Empty, self.activateGripper)
    ...
    def moveGripper(self, req):
        ...
        [success, has_object] = self.robot.send_gripper_move(req.pos, req.force, req.speed, req.position_compensation)
        return [success, has_object]

    def activateGripper(self, req):
        print "activateGripper"
        if not self.robot:
            return []

        self.robot.send_gripper_activate()
        return []
...
class URCartTrajectory(object):
    ...
    def execute_cb(self, goal_handle):
        ...
        for pose in self.cart_traj.poses:
            if(goal_handle.header.frame_id == "base"):
                p_ur_bl = pose
            else:
                ps = PoseStamped()
                ...
                ps_ur_bl = self.tfl.transformPose("base", ps)
            ...
            params = [MULT_jointstate * movel_speed,
                      ...
                      MULT_jointstate * vector_angle[0],
...
def main():
    ...
    global prevent_programming
    prevent_programming = rospy.get_param("/ur_driver/prevent_programming", False)
        ...
        while not rospy.is_shutdown():
            ...
            if getConnectedRobot(wait=False):
                ...
                while True:
                    ...
                    r = getConnectedRobot(wait=True, timeout=3.0) # increase timeout in case ur prog take too long to launch e.g. prog gets bigger
                    if r:
                        break
